...
int main()
{
  ...
  for (auto& robot_particle : p) {
    robot_particle.set_noise(Forward_Noise, Turn_Noise, Sense_Noise);
    // cout << robot_particle.show_pose() << endl;
  }
  ...
  Robot p2[n];
  for (int i = 0; i < n; i++) {
    //p2[i].set_noise(Forward_Noise, Turn_Noise, Sense_Noise);
